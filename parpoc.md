...
We need a way to distribute pl/sql libraries, like jars for java.
It sould be somenthing maven can use and possibly that can be hosted
on maven central.


# Details #

The main idea is to have a jar containing a single par.sql
  * The jar is a common file that can be handled by maven and repos

  * what we have inside the jar? a PAR! a what? a PAR: a fat PL/SQL
> > package containing a base64 encoded zip file, containing pl/sql
> > packages, java classes, java sources, java resources, xdb files,
> > and whatever.... (see http://erikwramner.wordpress.com/2010/02/23/coding-and-decoding-base64-in-plsql/ for zip handling in plsql)

  * why: because you don't need any external tool (loadjava for instance) to deploy it!
> > you can do it by running the par.sql via sqlplus or via jdbc,
...
```
  ...
  * self contained par POC:
```
create or replace package par_sample
as
...
as
     ...
     begin
       ...
       dbms_lob.writeappend(v_base64, 460, '');
       -- .... many of this rows ...
     ...
     begin
       null;
       -- based on the file hierarchy we can deploy 
       -- plsql/java/xdb and schema objects...
     end;

end;
```
  * those par packages can be generated by a PL/SQL package or by maven
  * can even be fetched from http and installed like any package manager software
  * git hub project for the POC https://github.com/aaaristo/par